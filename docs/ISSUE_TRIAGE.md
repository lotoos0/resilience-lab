...
| --- | --- | --- | --- | --- |
| #29 | `[v0.1.0] Expose rate-limit metrics and logs` | partial, v0.1.0 | Keep open until verified, then close. | API exposes `/metrics`; `rl_allowed_total` and `rl_denied_total` exist in `services/api/middleware/rate_limit.py`; k6 load tests exist. Need verification evidence and README usage notes. |
...
| #35 | `[v0.1.0] Verify Grafana dashboard provisioning` | partial, v0.1.0 | Keep open or close after deciding provisioning expectations. | `deploy/grafana/dashboards/system-overview.json` exists, plus screenshot. Need proof Grafana is installed/provisioned and admin access handled. |
